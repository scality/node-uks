...
var utils = {
  ...
  getNextReplica: function(value, cls1) {
    ...
    if(replica_schemes[clt] !== undefined) {
      ...
      nextKey = nextKey.value.shiftLeft(CHUNKID_EXTRA_BITS).or(
        ...
        entropyKey = k.value.shiftRight(CHUNKID_RAIN_EXTRA_BITS);
        scheme = utils.getRainSchema(k);
        nreplicas = utils.getRainK(k).add(getRainM(k));
        nextReplica = (replica + 1) % nreplicas;
        nextKey = utils.getNextReplicaInt(entropyKey,
                                            scheme,
                                            replica, nextReplica);
        // TODO: (from the docs) Bitwise operations (and, or, xor) are implemented for positive numbers only.
        /*
  ...
  getNextReplicaInt: function(entropyKey, cl, replica, nextReplica) {
    ...
    var nextKey = entropyKey.value.sub(toSubtract).add(toAdd);
  ...
  getNextRainReplicaInt: function(entropyKey, schema, replica, nextReplica) {
    ...
    var nextKey = entropyKey.value.sub(toSubtract)
                  .add(toAdd);
